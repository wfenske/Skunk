...
public class AnalyzedDataHandler {
    ...
    private String getFileSortedRestults(Map<FeatureReference, List<SmellReason>> results) {
        ...
        Collections.sort(sortedKeys, new ComparatorChain<FeatureReference>(FEATURECONSTANT_FILEPATH_COMPARATOR,
    ...
    private String getFeatureSortedResults(Map<FeatureReference, List<SmellReason>> results) {
        ...
        // sort the keys after featurename, filepath and start
        List<FeatureReference> sortedKeys = new ArrayList<FeatureReference>(results.keySet());
        Collections.sort(sortedKeys, new ComparatorChain<FeatureReference>(FEATURECONSTANT_FEATURENAME_COMPARATOR,
    ...
    private String getOverviewResults(Map<FeatureReference, List<SmellReason>> results) {
        ...
        res += "Number of features: \t" + constants.size() + " (" + percentOfConstants + "% of "
                + FeatureExpressionCollection.GetFeatures().size() + " constants)\r\n";
        res += "Number of feature constants: \t" + countLocations + " (" + percentOfLocations + "% of "
                + FeatureExpressionCollection.numberOfFeatureConstantReferences + " locations)\r\n";
        res += "Lines of annotated Code: \t" + completeLoac + " (" + percentOfLoc + "% of "
                + FeatureExpressionCollection.GetLoc() + " LOC)\r\n";
    ...
    private Object[] createFileRecord(de.ovgu.skunk.detection.data.File file) {
        ...
        // Loac/Loc * #FeatLocs
        float featLocSmell = conf.File_LoacToLocRatio_Weight
                * (((float) file.GetLinesOfAnnotatedCode() / (float) file.loc) * file.numberOfFeatureLocations);

        // #Constants/#FeatLocs
        float featConstSmell = conf.File_NumberOfFeatureConstants_Weight
                * ((float) file.GetFeatureConstantCount() / (float) file.numberOfFeatureLocations);

        // Loac/Loc * #FeatLocs
        float nestSumSmell = conf.Method_NestingSum_Weight
    ...
    private Object[] createFeatureRecord(Feature feat) {
        ...
        float constSmell = this.conf.Feature_NumberNofc_Weight * (((float) feat.getConstants().size())
                / ((float) FeatureExpressionCollection.numberOfFeatureConstantReferences));

        // LOFC/TotalLoc
        float lofcSmell = this.conf.Feature_NumberLofc_Weight
                * (((float) feat.getLofc()) / ((float) FeatureExpressionCollection.GetLoc()));
    ...
    /**
     * GCalculate the smell value of the current method and return data record
     * as list
     *
     * @param currentMethod
     *            the current method
    ...
    private Object[] createMethodRecord(Method currentMethod) {
        // calculate smell values
        // Loac/Loc * #FeatLocs
        float featLocSmell = conf.Method_LoacToLocRatio_Weight
                * (((float) currentMethod.GetLinesOfAnnotatedCode() / (float) currentMethod.loc)
                        * currentMethod.numberFeatureLocations);

        // #Constants/#FeatLocs
        float featConstSmell = conf.Method_NumberOfFeatureConstants_Weight
                * ((float) currentMethod.GetFeatureConstantCount() / (float) currentMethod.numberFeatureLocations);

        // Loac/Loc * #FeatLocs
        float nestSumSmell = conf.Method_NestingSum_Weight
                * ((float) currentMethod.nestingSum / (float) currentMethod.numberFeatureLocations);
        float sum = (featLocSmell + featConstSmell + nestSumSmell);
